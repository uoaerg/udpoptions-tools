...
func newScanner(ip net.IP) (*scanner, error) {
	...
	iface, err := net.InterfaceByName("em0")
	//gw := net.ParseIP("139.133.204.64")
	//src := net.ParseIP("139.13.204.4")

	gw := net.IPv4(139,133,204,64)
	src := net.IPv4(139,13,204,4)
...
func (s *scanner) getHwAddr() (net.HardwareAddr, error) {
	...
	log.Printf("gw: %+v", s.gw)
	log.Printf("iface addr: %+v", s.iface.HardwareAddr)

	log.Printf("dst: %v", s.src)
	log.Printf("arpDst: %v", arpDst)
	...
	arp := layers.ARP{
		...
		SourceProtAddress: []byte{139, 133, 204, 4}, 
		DstHwAddress:      []byte{0, 0, 0, 0, 0, 0},
		//DstProtAddress:    []byte(arpDst),
		DstProtAddress:    []byte{139, 133, 204, 64},
	}
